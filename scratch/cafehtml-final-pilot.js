...
async function runFinalPilot() {
  try {
    ...
    console.log("2. Uploading Product Images with Alt Text...");
    ...
    for (const imgData of imageFiles) {
      ...
      if (fs.existsSync(imgPath)) {
        console.log(`Uploading image ${imgData.name}...`);
        ...
        try {
          ...
          console.error(`❌ Failed to upload image ${imgData.name}:`, imgErr.response?.data || imgErr.message);
        }
      }
    }

    // 3. Upload Digital Download File
    console.log("3. Uploading Digital Download File...");
    ...
    if (fs.existsSync(digitalFilePath)) {
      ...
      fileForm.append("name", "Zero_Boyfriends_Digital.png");
      ...
      try {
        ...
        console.error("❌ Failed to upload digital file:", fileErr.response?.data || fileErr.message);
      }
    }

    // 4. Set Properties (Pink & Valentine's)
    console.log("4. Setting dynamic properties (Color & Occasion)...");
    try {
    ...
    console.log("\n=================================");
    console.log(`🎉 PERFECT DIGITAL LISTING PUSH COMPLETE!`);
