...
const AMAZON_NECK_STYLES = ["Crew Neck","V-Neck","Round Neck","Boat Neck","Halter Neck","High Neck","Hooded Neck","Mock Neck","Turtle Neck","Square Neck","Off Shoulder Neck","Asymmetric Neck"
];

const AMAZON_COLLAR_STYLES = ["Collarless","Button-Down","Spread","Mandarin","Polo"
];

const AMAZON_SLEEVE_LENGTHS = ["3/4 Sleeve","Bracelet Sleeve","Half Sleeve","Long Sleeve","Short Sleeve","Sleeveless"
];

const AMAZON_SLEEVE_TYPES = ["Balloon Sleeve","Batwing Sleeve","Bell Sleeve","Bishop Sleeve","Butterfly Sleeve","Cap Sleeve","Cape Sleeve","Cold Shoulder Sleeve","Cuff Sleeve","Dolman Sleeve","Flutter Sleeve","Gathered Sleeve","Kimono Sleeve","Lantern Sleeve","Long Sleeve","Puff Sleeve","Raglan Sleeve","Roll Up Sleeve","Short Sleeve","Sleeveless","Slit Sleeve","Strapless"
];

const AMAZON_SLEEVE_CUFFS = ["Rolled","Open","Ribbed Cuff","Barrel Cuff","French Cuff","Single Cuff"
];

const AMAZON_TARGET_GENDERS = ["unisex","female","male"
];

const AMAZON_AGE_RANGES = ["adult","teen","child","infant"
];

const AMAZON_DEPARTMENTS = ["women","men","unisex-adult","girls","boys"
];

const AMAZON_CARE_INSTRUCTIONS = ["Machine Wash","Hand Wash","Dry Clean Only","Do Not Wash","Line Dry"
];

const AMAZON_FABRIC_STRETCHES = ["high_stretch","low_stretch","medium_stretch","no_stretch"
];

const AMAZON_ITEM_LENGTH_DESCRIPTIONS = ["Extra Long Length","Extra Short Length","Long Length","Short Length","Standard Length"
];

const AMAZON_WEIGHT_CLASSES = ["heavyweight","lightweight","medium_weight"
];

const AMAZON_FASHION_DECADES = ["1900s","1910s","1920s","1930s","1940s","1950s","1960s","1970s","1980s","1990s","2000s","2010s"
];

const AMAZON_RECOMMENDED_BROWSE_NODES = ["Men's T-Shirts & Polos (1968120031)","Women's T-Shirts (1968255031)","Men's Casual Shirts (1968093031)","Men's Sweatshirts & Hoodies (1968123031)","Women's Shirts & Tops (1968253031)"
];

const AMAZON_GARMENT_SIZE_COUNTRIES = ["IN","US","GB","EU","JP","FR","DE","IT","CA","AU"
];


const AMAZON_TOP_STYLES = ["A-Line","Asymmetric","Blouson","Boxy","Bralette","Bustier","Camisole","Cape","Hooded","Kaftan","Layered","Peplum","Poncho","Pullover","Tube","Wrap"
];

const AMAZON_SHIRT_FORM_TYPES = ["T-Shirt","Button-Down","Polo","Henley","Tank Top"
];

const AMAZON_CLOSURE_TYPES = ["Button","Zipper","Snap","Pull On","Drawstring","Hook and Eye","Hook and Loop","Buckle"
];

const AMAZON_ITEM_TYPE_NAMES = ["T-Shirt","Shirt","Polo","Sweatshirt","Hoodie","Jacket","Tank Top"
];

const AMAZON_FABRIC_STRETCHABILITIES = ["stretchable","non_stretchable"
];
...
const AMAZON_SPECIAL_FEATURES = ["Moisture Wicking","Lightweight","Breathable","Water Resistant","Quick Dry"
];

const AMAZON_PATTERNS = ["Floral","Solid","Striped","Plaid","Polka Dot","Geometric","Animal Print"
];

const AMAZON_THEMES = ["Animals","Sports","Music","Movies","Holidays","Nature","Space"
];

const AMAZON_SUBJECT_CHARACTERS = ["Batman","Superman","Spider-Man","Mickey Mouse","Harry Potter","Star Wars"
];

const AMAZON_ANIMAL_THEMES = ["Alpaca","Cat","Dog","Dinosaur","Bear","Lion","Tiger"
];

const AMAZON_POCKET_DESCRIPTIONS = ["Basic-5-Pocket","Slant","Flap","Patch","Cargo","No Pocket"
];

const AMAZON_NUMBER_OF_POCKETS = ["1","2","3","4","5","6"
];

const AMAZON_SEASONS = ["Summer","Spring","Winter","Autumn","All-Season"
];

const AMAZON_EMBELLISHMENT_FEATURES = ["Buckle","Applique","Embroidery","Lace","Sequin","Beaded","Ruffle"
];
...
export default function AmazonPresetManagerModal({ onClose, selectedRowsCount, onApplyPreset }: AmazonPresetManagerModalProps) {
 ...
 <label className="text-xs font-semibold text-zinc-650">{label}{requiredMark(required)}</label>
 ...
 className="w-3.5 h-3.5 text-blue-600 border-zinc-300 focus:ring-blue-500 rounded-none"
 />
 Auto
 </label>
 </div>
 ...
 isAuto ?"opacity-60 bg-zinc-100 text-zinc-500":""
 }`}
 ...
 className="w-full px-3 py-2 text-sm rounded-none border border-zinc-300 bg-white text-zinc-900 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
 >
 <option value="Auto"className="bg-blue-100 text-blue-700 font-semibold">Auto (AI Deduces)</option>
 ...
 return (
 <div className="flex flex-col gap-1">
 <label className="text-xs font-semibold text-zinc-655">{label}{requiredMark(required)}</label>
 <input
 type="text"
 value={String(editForm[field] ??"")}
 onChange={(e) => setEditForm({ ...editForm, [field]: e.target.value })}
 className="w-full px-3 py-2 text-sm font-medium rounded-none border border-zinc-300 bg-white text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
 placeholder={placeholder}
 />
 </div>
 );
 ...
 return (
 <div className="flex flex-col gap-1">
 <label className="text-xs font-semibold text-zinc-655">{label}</label>
 <select
 ...
 <div className="bg-white border border-zinc-300 w-full max-w-5xl flex flex-col overflow-hidden shadow-2xl"style={{ height:"85vh", minHeight:"650px", maxHeight:"900px"}} onClick={e => e.stopPropagation()}>
 
 {/* Header */}
 <div className="flex items-center justify-between p-4 border-b border-zinc-300 bg-zinc-50 shrink-0">
 <h2 className="text-sm font-bold text-zinc-900 uppercase tracking-wider">Amazon Listing Presets</h2>
 <button onClick={handleClose} className="p-1 hover:bg-zinc-200 text-zinc-500 rounded-none">
 <X size={18} />
 </button>
 </div>
 ...
 <div className="p-3 border-b border-zinc-300 shrink-0">
 <button 
 onClick={handleCreateNew}
 className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-semibold bg-white text-zinc-800 border border-zinc-300 hover:bg-zinc-100 rounded-none shadow-sm uppercase tracking-wider"
 >
 ...
 <h4 className={`text-xs font-bold truncate uppercase tracking-wider ${activePresetId === p.id ?'text-blue-600':'text-zinc-850'}`}>
 {p.name}
 </h4>
 ...
 {editForm ? (
 <>
 <div className="p-3 border-b border-zinc-200 bg-zinc-50/50 shrink-0">
 ...
 className="px-2 py-1 text-xs border border-zinc-300 bg-white text-zinc-900 rounded-none focus:outline-none focus:ring-1 focus:ring-blue-500 font-semibold max-w-sm"
 />
 </div>
 </div>
 ...
 <label className="text-xs font-semibold text-zinc-650">Sport Type (up to 2)</label>
 <button type="button"onClick={() => setIsSportTypeOpen(!isSportTypeOpen)} className="w-full px-3 py-2 text-sm border border-zinc-300 bg-white text-zinc-900 flex justify-between items-center min-h-[38px]">
 ...
 <div className="absolute bottom-full left-0 mb-1 w-full bg-white border border-zinc-300 shadow-lg z-20 max-h-60 overflow-y-auto p-2 space-y-1">
 ...
 return <label key={option} className="flex items-center gap-2 px-2 py-1.5 hover:bg-zinc-100 cursor-pointer text-sm">
 ...
 <div className="space-y-4">
 <div>
 <label className="block text-xs font-semibold text-zinc-700 mb-1">AI Context</label>
 <textarea value={editForm.context} onChange={(e) => setEditForm({ ...editForm, context: e.target.value })} className="w-full min-h-24 px-3 py-2 text-xs border border-zinc-300 bg-white text-zinc-900 focus:outline-none focus:ring-1 focus:ring-blue-500"/>
 </div>
 {([
 ...
 <textarea value={editForm[field]} onChange={(e) => setEditForm({ ...editForm, [field]: e.target.value })} className="w-full min-h-20 px-3 py-2 text-xs border border-zinc-300 bg-white text-zinc-900 focus:outline-none focus:ring-1 focus:ring-blue-500"/>
 </div>
 ))}
 </div>
 )}
 ...
 <select
 className="text-xs px-2 py-1 rounded-none border border-zinc-300 bg-white text-zinc-800 focus:outline-none"
 onChange={(e) => {
 ...
 <div className="text-xs font-bold text-zinc-700">{prop.name}</div>
 
 <div>
 <span className="block text-[10px] font-bold uppercase text-zinc-400 mb-1">Add Suggestions:</span>
 <div className="flex flex-wrap gap-1 border border-zinc-150 p-1.5 bg-zinc-55 mb-2">
 ...
 className="text-[10px] bg-white hover:bg-blue-50 border border-zinc-200 px-1.5 py-0.5 text-zinc-750"
 >
 + {opt}
 </button>
 ))}
 ...
 className="w-full px-2 py-1 text-xs rounded-none border border-zinc-300 bg-white focus:outline-none"
 onKeyDown={(e) => {
 if (e.key ==="Enter") {
 e.preventDefault();
 ...
 <div className="max-h-60 overflow-y-auto border border-zinc-200">
 ...
 className="w-full px-1.5 py-0.5 border border-zinc-305 bg-white text-xs text-zinc-900 focus:outline-none"
 />
 </td>
 <td className="p-2">
 <input
 ...
 className="w-full px-1.5 py-0.5 border border-zinc-305 bg-white text-xs text-zinc-800 focus:outline-none"
 />
 </td>
 </tr>
 );
